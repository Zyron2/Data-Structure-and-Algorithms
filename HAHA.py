...
def write_to_file(x_values, y_values_dict):
    with open("finaldata.txt", "w") as file:
        for expression_name, values in y_values_dict.items():
            file.write(expression_name + ": ")
            file.write(", ".join(map(str, values)))  # Join values with comma #make space every problem
            file.write("\n\n")
...
while True:
    # Prompt user to choose an expression or solve all
    try:
        choice = int(input("\nEnter the number of the expression you want to plot (or 11 to solve all, 12 to exit): "))
        
        if choice == 12: # the program will ask the user if they want to continue by entering another expression number, 
                            #enter -1 to exit the program, or enter 0 to solve all expressions. 
                            #If the user enters an invalid choice, the program will prompt again until a valid choice is made.
            ...
            print("Invalid choice. Please enter a number between -1 and", len(expressions))
